        ...
        if (query.HasRows == true)
        {
            ...
            for (var i = 0; i < dt.Rows.Count; i++)
            {
                ...
                foreach (var col in dt.Columns)
                {
                    ...
                    var innerHtml = "<a href='Edit_Record.aspx?id=" + row.ItemArray[0] + "' target='_blank'><div>" + content + "</div></a>";
        ...
        if (query.HasRows == true)
        {
            ...
            for (var i = 0; i < dt.Rows.Count; i++)
            {
                ...
                foreach (var col in dt.Columns)
                {
                    ...
                    var innerHtml = "<a href='AddFromLog.aspx?id=" + row.ItemArray[0] + "' target='_blank'><div>" + content + "</div></a>";
            ...
            //using (TextFieldParser parser = new TextFieldParser(file)
            {
                ...
                while (true)
                {
                    ...
                    if (parts[0] != "SerialNumber")
                    {
                        ...
                        //Added new fields according to the sp_Inventory_AddNewTempTable procedure on 2/5/2016
                        ...
                        //formatting month name to number
                        try
                        {
                            DateTime pDate = DateTime.Parse(purchaseDate);
                            purchaseDate= pDate.ToString("MM/dd/yyyy");
                        }
                        catch(Exception err1)
                        {
                            ClientScript.RegisterClientScriptBlock(this.GetType(), "btnRun","sweetAlert('Oops','Purchase Date at row "+count+" is not in the acceptable date format MM/DD/YYYY.'+'\\n\\n'+'Please modify the value','error');",true);
                            return;
                        }

                        try
                        {
                            DateTime aDate = DateTime.Parse(auxComputerDate);
                            auxComputerDate = aDate.ToString("MM/dd/yyyy");
                        }
                        catch(Exception err2)
                        {
                            ClientScript.RegisterClientScriptBlock(this.GetType(), "btnRun","sweetAlert('Oops','Aux Computer Date at row "+count+" is not in the acceptable date format MM/DD/YYYY.'+'\\n\\n'+'Please modify the value','error');",true);
                            return;
                        }
                        ...
                        if (reader1.HasRows == true)
                        {
                            ...
                            string[] headers = new string[]{"serialnum","make","model","computerName","type","category","user","userPawprint","department","department","location","roomLetter","building","note","ownedBy","purchaseDate","auxComputerDate"};
                            //Parsing uploaded data to check values have acceptable lengths
                            for (int k = 1; k < 16; k++)
                            {

                                StringBuilder script = new StringBuilder();
                                if (parts[k].ToString().Length > 255)
                                {

                                    ClientScript.RegisterClientScriptBlock(this.GetType(), "btnRun","sweetAlert('Oops','The column "+ headers[k]+" at row "+count+" with "+parts[k].Length+" characters is exceeding acceptable number(255) of characters.'+'\\n\\n'+'Please modify the value','error');",true);
                                    return;
                                }
                            ...
                            for (int j = 0; j <4; j++)
                            {
                                ...
                                if (valuefromdb != valuefromreader)
                                {
                                    ...
                                    cmd1.CommandText = "[dbo].[sp_Inventory_AddNewTempTable]"; //Created a new  procedure sp_Inventory_AddNewTempTable instead of sp_Inventory_AddTempTable to store the records
                            ...
                            if (flag == 0)
                            {
                                ...
                                com3.CommandText = "update [dbo].[ComputerInventory] set LastVerified = SYSDATETIME() where SerialNumber = '" + serialnum + "'";
                                com3.ExecuteScalar();
                            }
                        ...
                        else
                        {
                            ...
                            cmd1.CommandText = "[dbo].[sp_Inventory_AddNewTempTable]"; //Created a new temp table sp_Inventory_AddNewTempTable instead of sp_Inventory_AddTempTable to store the records
                            cmd1.Parameters.AddWithValue("@SerialNumber", serialnum);
                            cmd1.Parameters.AddWithValue("@Make", make);
                            cmd1.Parameters.AddWithValue("@Model", model);
                            cmd1.Parameters.AddWithValue("@ComputerName", computerName);
